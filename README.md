...
**Project 3 — Following the Money.** *In progress.* Investigated why Breaking Games' data can't support a formal attribution model (no UTM tags, no session-to-purchase link — correlation only, stated explicitly rather than overstated). Designed and specified a 6-channel taxonomy for `dim_referrer` — Paid Social, Organic Search, Paid Search, Organic Social, Owned, Other — validated against the real traffic split before building it:

| Channel | Share of pageviews |
|---|---|
| Owned (direct + email) | 46.7% |
| Paid Social (Meta) | 27.3% |
| Organic Search | 24.2% |
| Other (long tail, ~388 referrers) | 1.4% |
| Organic Social | 0.3% |
| Paid Search (non-Meta) | 0.3% |

Next: the Q4 ad allocation plan built on top of this taxonomy. See `Design-referral-table-process.docx` for the full process behind this table.
